...
class UserAuth
{
    ...
    {
		// 边栏开始
		Hook::listen("user_sidebar_begin", $params);    $colorArr = ['red', 'green', 'yellow', 'blue', 'teal', 'orange', 'purple'];        $colorNums = count($colorArr);
        ...
        /*$indexRuleList = \app\common\model\UserRule::where('status', 'normal')
            ->where('ismenu', 0)
    ...
    public function getMenuList()
    {
        ...
        $rules = explode(',', $group->rules);
        $this->menus = collection(UserRule::where('status', 'normal')
            ->where('id', 'in', $rules)->where('ismenu', 1)
            ->order('weigh', 'desc')
            ->cache("__usermenu__")
            ->field('id,pid,name,title,ismenu,status,icon')
            ->select())->toArray();
        return $this->menus;
    }
